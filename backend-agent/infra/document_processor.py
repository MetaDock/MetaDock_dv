...
class DocumentProcessor:
    def __init__(self, 
                 docs_path: List[str] = None,  # default from config or data/help_pages_for_test 
        ...
        if docs_path is None:
            try:
                import config as _c
                docs_path = [_c.HELP_PAGES_DIR]
            except Exception:
                _root = Path(__file__).resolve().parent.parent.parent
                docs_path = [str(_root / 'data' / 'help_pages_for_test')]
        for path in docs_path:
            if path.startswith("../"):
                self.docs_paths.append(Path(__file__).parent.parent.parent / path[3:])
            else:
                self.docs_paths.append(Path(path))
        if vector_store_path == "./vector_store":
            try:
                import config as _c
                vector_store_path = _c.VECTOR_STORE_PATH
            except Exception:
                pass
