...
# Ensure backend-agent is on path for core/infra/prompt_loader
_backend_agent = Path(__file__).resolve().parent.parent
if str(_backend_agent) not in sys.path:
    sys.path.insert(0, str(_backend_agent))

from core.bioinfo_agent import BioinfoAgentV2 as BioinfoAgent
...
def ensure_vector_store():
    ...
    try:
        ...
        vector_store_path = Path(__file__).resolve().parent.parent.parent / 'vector_store'
...
def initialize_agent():
    ...
    if agent is None:
        ...
        try:
            ...
            # Initialize Agent with specified memory store path (use env or default under project)
            try:
                from pathlib import Path
                _root = Path(__file__).resolve().parent.parent
                memory_store_path = os.environ.get("MEMORY_STORE_PATH") or str(Path(__file__).resolve().parent.parent / 'memory_store')
                if not os.path.isdir(memory_store_path):
                    memory_store_path = os.environ.get("MEMORY_STORE_PATH") or str(Path(__file__).resolve().parent.parent / 'memory_store')
            except Exception:
                memory_store_path = os.environ.get("MEMORY_STORE_PATH", os.path.join(os.path.dirname(__file__), "memory_store"))
...
def get_supported_models():
    ...
    try:
        from infra.agent_client import MultiLLMClient, get_llm_client
...
def switch_model():
    ...
    try:
        ...
        from infra.agent_client import switch_global_model, get_llm_client
...
def test_model():
    ...
    try:
        ...
        from infra.agent_client import MultiLLMClient
